...
static void DoSmooth(void)
{
    ...
    for (x = 0; x < WORLD_X/2; x++) {
        for (y = 0; y < WORLD_Y/2; y++) {
            ...
            if (x > 0) z += tem[x - 1][y];
            if (x < (WORLD_X/2 - 1)) z += tem[x + 1][y];
            if (y > 0) z += tem[x][y - 1];
            if (y < (WORLD_Y/2 - 1)) z += tem[x][y + 1];
...
static void DoSmooth2(void)
{
    ...
    for (x = 0; x < WORLD_X/2; x++) {
        for (y = 0; y < WORLD_Y/2; y++) {
            ...
            /* Get average of nearby cells */
            if (x > 0) z += tem2[x - 1][y];
            if (x < (WORLD_X/2 - 1)) z += tem2[x + 1][y];
            if (y > 0) z += tem2[x][y - 1];
            if (y < (WORLD_Y/2 - 1)) z += tem2[x][y + 1];
...
static void SmoothPSMap(void)
{
    ...
    for (x = 0; x < WORLD_X/4; x++) {
        for (y = 0; y < WORLD_Y/4; y++) {
            ...
            if (x > 0) edge += PoliceMap[x - 1][y];
            if (x < (WORLD_X/4 - 1)) edge += PoliceMap[x + 1][y];
            if (y > 0) edge += PoliceMap[x][y - 1];
            if (y < (WORLD_Y/4 - 1)) edge += PoliceMap[x][y + 1];
...
static void SmoothFSMap(void)
{
    ...
    for (x = 0; x < WORLD_X/4; x++) {
        for (y = 0; y < WORLD_Y/4; y++) {
            ...
            /* Add up surrounding cells */
            if (x > 0) edge += FireStMap[x - 1][y];
            if (x < (WORLD_X/4 - 1)) edge += FireStMap[x + 1][y];
            if (y > 0) edge += FireStMap[x][y - 1];
            if (y < (WORLD_Y/4 - 1)) edge += FireStMap[x][y + 1];
...
static void SmoothTerrain(void)
{
    ...
    for (x = 0; x < WORLD_X/4; x++) {
        for (y = 0; y < WORLD_Y/4; y++) {
            ...
            if (x > 0) z += Qtem[x - 1][y];
            if (x < (WORLD_X/4 - 1)) z += Qtem[x + 1][y];
            if (y > 0) z += Qtem[x][y - 1];
            if (y < (WORLD_Y/4 - 1)) z += Qtem[x][y + 1];
...
static int GetPValueLocal(int loc)
{
    ...
    if (loc < POWERBASE) {
        if (loc >= ROADBASE+16) return 75;   /* Heavy traffic */
        if (loc >= ROADBASE) return 50;      /* Light traffic */
        if (loc < ROADBASE) {
            if (loc > FIREBASE) return 90;    /* Fire? */
            if (loc >= RADTILE) return 255;   /* Radioactive */
        }
        return 0;
    }
    ...
    if (loc < PORTBASE) return 50;           /* Industry */
    if (loc <= POWERPLANT+10) return 100;    /* Port, airport, power plant */
...
static int GetPDen(int zone)
{
    ...
    /* Calculate population based on zone type */
    if (zone < COMBASE) {
    ...
    if (zone < INDBASE) {
        ...
        pop = calcComPop(zone) << 3;
        return pop;
    }
    if (zone < PORTBASE) {
        /* Industrial population (higher weight) */
        pop = calcIndPop(zone) << 3;
        return pop;
    }
